...
export default function ExperienceSection(): React.ReactElement {
  ...
  const headingUnderlineWidthRaw: MotionValue<number> = useSpring(
    ...
    <React.Fragment>
      ...
      >
        ...
        <div className={`flex-1/3 h-full !px-[7rem] !pt-[7rem]`}>
          ...
          {experiences.map(
            (item: ExperienceProps, index: number): React.ReactElement => {
              return (
                <SingleLineExperienceDisplay
                  item={item}
                  index={index}
                  key={index}
                />
          ...
          {accomplishments.map(
            (item: AccomplishmentsProps, index: number): React.ReactElement => {
              return (
                <SingleLineAccomplishmentDisplay
                  item={item}
                  index={index}
                  key={index}
                />
...
function SingleLineExperienceDisplay({
  item,
  index,
}: {
  ...
  const scale: MotionValue<number> = useSpring(
    useTransform(scrollYProgress, [0, 1], [0.8, 1]),
    springOptions,
  );
  const y: MotionValue<number> = useSpring(
...
function SingleLineAccomplishmentDisplay({
  item,
  index,
}: {
